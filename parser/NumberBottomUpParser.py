...
class NumberBottomUpParser(object):
    ...
    def get_agenda_permutations(self):
        ...
        permutations = [substring_dict[str(NumberMinimalistGrammarTree.EMPTY_STRING)] + list(permutation) for permutation in product(*possible_words)]
        return permutations
    ...
    def get_trigger_item(self, agenda):
        max_item = agenda[0]
        for item in agenda:
            if sum([j - i for i, j in sum([node.substring for node in item.nodes], [])]) > sum([j - i for i, j in sum([node.substring for node in max_item.nodes], [])]):
                max_item = item
    ...
    def translate_to_strings(self, number_tree):
        ...
        for node in number_tree.nodes:
            if node.substring == NumberMinimalistGrammarTree.EMPTY_STRING:
                ...
                substring = MinimalistGrammarTree.SUBSTRING_DELIMITER.join(self.input_substrings[node.substring[0][0] : node.substring[0][1]])
    ...
    def init_goals(self, input):
        self.goals = [NumberMinimalistGrammarTree.NumberMinimalistGrammarTree([NumberMinimalistGrammarTree.NumberMinimalistGrammarNode([(0, self.input_length)], self.categories["IP"], [], [], [])], NumberMinimalistGrammarTree.TYPE_SIMPLE, "right"),
                      NumberMinimalistGrammarTree.NumberMinimalistGrammarTree([NumberMinimalistGrammarTree.NumberMinimalistGrammarNode([(0, self.input_length)], self.categories["IP"], [], [], [])], NumberMinimalistGrammarTree.TYPE_COMPLEX, "right"),
                      NumberMinimalistGrammarTree.NumberMinimalistGrammarTree([NumberMinimalistGrammarTree.NumberMinimalistGrammarNode([(0, self.input_length)], self.categories["IP"], [], [], [])], NumberMinimalistGrammarTree.TYPE_SIMPLE, "left"),
                      NumberMinimalistGrammarTree.NumberMinimalistGrammarTree([NumberMinimalistGrammarTree.NumberMinimalistGrammarNode([(0, self.input_length)], self.categories["IP"], [], [], [])], NumberMinimalistGrammarTree.TYPE_COMPLEX, "left")]
...
if __name__ == '__main__':
    ...
    from Grammars import ENGLISH_GRAMMAR, HEAD_FINAL_GRAMMAR
    
    input = ['George wrote Kramer and Kramer above', 'Jerry and Elaine Jerry loved', 'George walked and Elaine read Elaine with', 'Elaine Jerry saw Jerry by and Jerry Elaine hated Jerry under', 'Jerry Elaine read that knows and Jerry Kramer hated', 'George and Jerry read', 'Kramer read', 'Kramer walked Kramer with', 'Elaine walked and Kramer Jerry liked Jerry by', 'George George ran that thinks and walked', 'Elaine read', 'George wrote George and Kramer with', 'Jerry wrote Jerry and George with', 'George read and Kramer Elaine loved', 'Elaine read and Elaine Kramer wrote that assumes Elaine above', 'Elaine Jerry Jerry liked George under that says and ran', 'George read and Elaine read', 'Jerry ran and Kramer wrote Elaine by', 'Kramer Elaine walked that says and Kramer George Elaine saw that assumes Kramer above', 'George wrote and walked', 'Jerry George and George walked that says George under', 'Elaine and Elaine wrote', 'George ran', 'Jerry and Jerry walked', 'Kramer and Elaine ran George under', 'George George loved George above and George ran', 'Elaine and Kramer Kramer saw', 'Kramer ran', 'Kramer read and Jerry loved', 'Jerry ran and Jerry wrote Kramer by', 'Jerry and Kramer read', 'Kramer walked Jerry with', 'Jerry wrote Elaine and George with', 'George Kramer George liked that thinks and George Kramer hated', 'Jerry George saw Elaine above and Kramer under', 'Jerry ran George and Kramer under', 'Kramer Jerry Kramer liked that assumes', 'Jerry Elaine hated and Kramer read', 'Elaine and Kramer George saw', 'Jerry Jerry and Kramer liked', 'Kramer walked and Jerry Jerry liked', 'George and Elaine read', 'Elaine ran and wrote Jerry under', 'Elaine read', 'George walked and George Elaine loved George with', 'Elaine wrote', 'Jerry wrote and Kramer saw', 'Elaine wrote and Elaine liked', 'Jerry and Kramer ran', 'Elaine Kramer liked']
