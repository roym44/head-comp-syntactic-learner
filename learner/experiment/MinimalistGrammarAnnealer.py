...
class AnnealerException(Exception):
    pass
...
class MinimalistGrammarAnnealer(object):
    ...
    def __init__(self, logger, initial_input, blank_grammar, learner_type, algorithm, separator=SUBSTRING_DELIMITER):
        ...
        self.initial_input_parsing_dict = {}
    ...
    def split_to_words(self):
        input_string = self.separator.join(self.initial_input)
        if self.separator == '':
    ...
    def add_probable_and_delete(self, hypothesis):
        for i in range(10):
            self.logger.info("Trying to add probable and delete")
    ...
    def add_any_lexical_item(self, hypothesis):
        self.logger.info("Add lexical item:")
        blank_lexicon_without_empty_string = [category for category in self.blank_grammar.lexicon if
                                              EMPTY_STRING not in str(category)]
        for i in range(10):
    ...
    def add_probable_lexical_item(self, hypothesis):
        self.logger.info("Add probable item:")
        blank_lexicon_without_empty_string = [category for category in self.blank_grammar.lexicon if
                                              EMPTY_STRING not in str(category)]
        hypothesis_without_substrings = [str(category).split(":")[1] for category in hypothesis.lexicon]
        probable_categories = [category for category in blank_lexicon_without_empty_string if
                               str(category).split(":")[1] in hypothesis_without_substrings]
        for i in range(10):
    ...
    def delete_impossible_category(self, hypothesis):
        ...
        self.logger.info("Testing the category " + str(category_to_test))
        ...
        try:
            ...
            return None, None
        self.logger.info("Removing impossible category: " + str(category_to_test))
        return new_grammar, energy
    ...
    def change_category_direction_with_flip(self, hypothesis):
        ...
        categories = set([item.head.base for item in hypothesis.lexicon if
                          item.head.substring != EMPTY_STRING and len(item.head.selects) == 1])
        category_to_flip = random.choice(list(categories))
        category_items = [item for item in hypothesis.lexicon if
                          item.nodes[0].base == category_to_flip and item.nodes[0].substring != EMPTY_STRING and len(
                              item.head.selects) < 2]
    ...
    def change_word_direction_with_flip(self, hypothesis):
        ...
        # Get all words.
        words = set([item.head.substring for item in hypothesis.lexicon if
                     item.head.substring != EMPTY_STRING and len(item.head.selects) < 2])
        word_to_flip = random.choice(list(words))
        word_items = [item for item in hypothesis.lexicon if
                      item.head.substring == word_to_flip and len(item.head.selects) < 2]
        ...
        if current_direction == "right":
            ...
            raise AnnealerException(
                "Invalid merge direction in hypothesis - word: %s, direction: %s" % (word_to_flip, current_direction))

        # We only bother to flip items with selects features - otherwise the direction doesn't matter.
        item_to_flip = random.choice(
            [item for item in hypothesis.lexicon if item.head.substring != EMPTY_STRING and item.head.selects])
    ...
    def energy(self, hypothesis, deleted=None, added=None, flipped_word=None, log=True):
        ...
        if log:
            self.logger.info(
                "Total number of possible derivations: " + str(sum([len(results) for results in parsing_results])))

        grammar_length = self.get_grammar_length(parsing_grammar)
        input_length = self.get_input_length(parsing_grammar, parsing_results)
        if log:
            self.logger.info("Grammar: %s" % (grammar_length,))
            self.logger.info("Input: %s" % (input_length,))
